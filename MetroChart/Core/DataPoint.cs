namespace MetroChart
{
    ...
    public class DataPoint : DependencyObject, INotifyPropertyChanged
    {
        ...
        public static readonly DependencyProperty MaxDataPointAutoProperty =
           ...
           typeof(double),
           typeof(DataPoint),
           new PropertyMetadata(0.0, new PropertyChangedCallback(MaxDataPointAutoChanged)));

        public static readonly DependencyProperty MaxDataPointDevideProperty =
           DependencyProperty.Register("MaxDataPointDevide",
           typeof(double),
           typeof(DataPoint),
           new PropertyMetadata(0.0, new PropertyChangedCallback(MaxDataPointDevideChanged)));
        ...
        public double PercentageFromMaxDataPointAuto
        {
            get
            {
                return MaxDataPointAuto;
            }
        }

        public double PercentageFromMaxDataPointDevide
        {
            get
            {
                return MaxDataPointDevide;
            }
        }
        ...
        private static void MaxDataPointAutoChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            (d as DataPoint).MaxDataPointAutoChanged(double.Parse(e.NewValue.ToString()));
        }

        private void MaxDataPointAutoChanged(double p)
        {
            RaisePropertyChangeEvent("PercentageFromMaxDataPointAuto");
        }

        private static void MaxDataPointDevideChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            (d as DataPoint).MaxDataPointDevideChanged(double.Parse(e.NewValue.ToString()));
        }

        private void MaxDataPointDevideChanged(double p)
        {
            RaisePropertyChangeEvent("PercentageFromMaxDataPointDevide");
        }
        ...
        private string GetItemValue(object item, string propertyName)
        {
            if (item != null)
            {
                ...
                throw new Exception(string.Format("MaxDataDivide '{0}' not found on item of type '{1}'", propertyName, item.GetType().ToString()));
            }
            return null;
        }
