namespace MetroChart
{
    ...
    {        
        ...
        {
            SetValue(RadialGaugePiece.FormattedAnimatedValueProperty, AnimatedValue.ToString("F1")); // romee f0  소숫점 첫번째 까지 처리 
        }
        ...
        {    
            try
            {
                ...
                // 게이지의 폭 조정 처리 
                Geometry segmentPathData = LayoutSegment(startAngle, endAngle, radius, 0.75, center, true);
                ...
                // 게이지의 폭 조정 처리 
                Geometry segmentPathDataBackground = LayoutSegment(endAngle, 360, radius, 0.75, center, true);
