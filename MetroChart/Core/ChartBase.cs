namespace MetroChart
{
    ...
    public abstract class ChartBase : Control, INotifyPropertyChanged
    {
        ...
        public static readonly DependencyProperty MaxDataPointAutoProperty =
             DependencyProperty.Register("MaxDataPointAuto",
             typeof(double), typeof(ChartBase), new PropertyMetadata(0.0, OnMaxDataPointAutoChanged));

        public static readonly DependencyProperty MaxDataPointDevideProperty =
             DependencyProperty.Register("MaxDataPointDevide",
             typeof(double), typeof(ChartBase), new PropertyMetadata(0.0, OnMaxDataPointDevideChanged));
        ...
        /// <remarks>MaxDataPointValue Divide Value</remarks>
        public double MaxDataPointDevide
        {
            get { return (double)GetValue(MaxDataPointDevideProperty); }
            set { SetValue(MaxDataPointDevideProperty, value); }
        }                  

        /// <remarks>MaxDataPoint calc Auto</remarks>
        public double MaxDataPointAuto
        {
            get { return (double)GetValue(MaxDataPointAutoProperty); }
            set { SetValue(MaxDataPointAutoProperty, value); }
        }
        ...
        protected void UpdateGridLines()
        {
            ...
            if (MaxDataPointAuto == 0)
            {
                for (var i = distance; i <= GridLinesMaxValue; i += distance)
                {
                    // 2016.08.31 romee add 소수점 이하 버리기 
                    string t1;
                    t1 = i.ToString();
                    if(t1.IndexOf(".") == -1)
            ...
            else  // 2014.07.11 romee add  
            {
                distance = MaxDataPointDevide; 
                for (var i = distance; i <= MaxDataPointValue; i += distance)
                {
                    gridLines.Add(i.ToString());
                }
            }
                    ...
                    if (GetIsRowColumnSwitched())
                    {
                        ...
                        foreach (ChartSeries initialSeries in this.Series)
                        {
                            ...
                            {                                
                                ...
                                //Console.WriteLine("seriesItemCaption : {0} , {1}", seriesItemCaption, itemIndex);
                            }
                        }
                            ...
                            foreach (ChartSeries allSeries in this.Series)
                            {
                                ...
                                foreach (var seriesItem in allSeries.Items)
                                {
                                    ...
                                    if (existingDataPoint == null)
                                    {
                                        ...
                                        dataPointGroup.DataPoints.Add(datapoint);
                                        //Console.WriteLine("seriesItemCaption : {0} , {1}", seriesItemCaption, seriesIndex); 
                                    }
                                    seriesIndex++;
                                }
                            }
...
//                int index = 0;
        ...
        private void CreateDataPointBindings(DataPoint datapoint, DataPointGroup dataPointGroup)
        {
            ...
            //Sende an Datapoints the maximalvalue des Charts mit (wichtig in clustered Column chart)
            var maxDataPointAutoBinding = new Binding();
            maxDataPointAutoBinding.Source = this;
            maxDataPointAutoBinding.Mode = BindingMode.OneWay;
            maxDataPointAutoBinding.Path = new PropertyPath("MaxDataPointAuto");
            BindingOperations.SetBinding(datapoint, DataPoint.MaxDataPointAutoProperty, maxDataPointAutoBinding);

            //Sende an Datapoints the maximalvalue des Charts mit (wichtig in clustered Column chart)
            var maxDataPointDevideBinding = new Binding();
            maxDataPointDevideBinding.Source = this;
            maxDataPointDevideBinding.Mode = BindingMode.OneWay;
            maxDataPointDevideBinding.Path = new PropertyPath("MaxDataPointDevide");
            BindingOperations.SetBinding(datapoint, DataPoint.MaxDataPointDevideProperty, maxDataPointDevideBinding);
        ...
        private static void OnMaxDataPointAutoChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            (d as ChartBase).OnMaxDataPointAutoChanged((double)e.NewValue);
        }
        protected virtual void OnMaxDataPointAutoChanged(double p)
        {

        }
        private static void OnMaxDataPointDevideChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            (d as ChartBase).OnMaxDataPointDevideChanged((double)e.NewValue);
        }
        protected virtual void OnMaxDataPointDevideChanged(double p)
        {
