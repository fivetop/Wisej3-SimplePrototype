...
namespace CosineKitty.ZeroConfigWatcher
{
    public class TrafficMonitor : IDisposable
    {
        ...
        private readonly Queue<Packet> inQueue = new Queue<Packet>();
        ...
        private static string dst_ipv4 { get; set; } = "224.0.0.251";
        private static int dst_port { get; set; } = 5353;

        public bool Sett(string ip , int port)
        {
            dst_ipv4 = ip;
            dst_port = port;
            return true;
        ...
        {
            var broadcastEndpoint = new IPEndPoint(IPAddress.Parse(dst_ipv4), dst_port);
            lock (mutex)
            {
                foreach (UdpClient client in clientList)
        ...
        private static UdpClient[] MakeClientList()
        {
            ...
            foreach (NetworkInterface adapter in adapterList)
            {
                ...
                //client.ExclusiveAddressUse = false;
                var localEp = new IPEndPoint(IPAddress.Any, dst_port);
                socket.Bind(localEp);
                var multicastAddress = IPAddress.Parse(dst_ipv4);
